...
class TableController extends Controller
{
    ...
    public function showTableSize(Request $request)
    {

        try {
            $table = $this->table->showTableSize();
            return ResponseService::success(" ", $table);
        } catch (Throwable $exception) {
            return ResponseService::error($exception->getMessage(), 'An error occurred');
        }
    }
    public function showTableResive(Request $request)
    {

        try {
            $table=Table::where('reservation_status',1)->get();
            return ResponseService::success(" ", $table);
        } catch (Throwable $exception) {
            return ResponseService::error($exception->getMessage(), 'An error occurred');
        }
